...
export class AppComponent {
  ...
  async ngOnInit(): Promise<void> {
    ...
    const table1 = await this.worker.table(data);

    console.log(table1.columns())
    ...
    const table2 = await this.worker.table(schema);

    table1.size().then(e => console.log(e))
  }
  ...
  async ngAfterViewInit(): Promise<void> {
    ...
    const table = await this.worker.table(data);

    const viewer1 = document.getElementById("view1") as any;
  ...
  // async initTableOnly() {
  //   let el = document.getElementById("tableOnly") as any;
  //   el.load(this.table);
  //   el.restore({
  //     "row_pivots": [
  //       "Distributor",
